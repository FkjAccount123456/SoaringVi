...
void _hashmap_insert(hashmap *obj, char *key, void *val) {
    ...
    do {
        if (obj->table[i] == -1) {
            ...
            obj->table[i] = obj->len++;
            return;
        } else if (obj->entrys[obj->table[i]].hash == hash && !strcmp(obj->entrys[obj->table[i]].key, key)) {
...
void _hashmap_insert_shortstr(hashmap *obj, size_t hash, void *val) {
    ...
    do {
        if (obj->table[i] == -1) {
            ...
            obj->table[i] = obj->len++;
            return;
        } else if (obj->entrys[obj->table[i]].len == 0 && obj->entrys[obj->table[i]].hash == hash) {
            obj->entrys[obj->table[i]].val = val;
            return;
        }
        i = (i + 1) % obj->mod;
...
void **hashmap_get_shortstr(hashmap *obj, size_t hash) {
    ...
    do {
        if (obj->table[i] == -1)
            return NULL;
        else if (obj->entrys[obj->table[i]].len <= SHORTSTR_MAX && obj->entrys[obj->table[i]].hash == hash)
            return &obj->entrys[obj->table[i]].val;
        i = (i + 1) % obj->mod;
    } while (i != pos);
    return NULL;
